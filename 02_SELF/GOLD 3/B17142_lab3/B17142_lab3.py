...
def spread(choice, arr, zeros):
    ...
    while q:
        ...
        for di, dj in [(-1, 0), (1, 0), (0, -1), (0, 1)]:
            ni, nj = ci + di, cj + dj
            # 인덱스 밖, 벽, 활성 바이러스를 만난 경우 건너뛰기
            # 벽, 이미 지나간 곳, 활성 바이러스가 있는 곳은 건너 뛰기
            if ni < 0 or ni >= n or nj < 0 or nj >= n or (arr[ni][nj] > 0 or arr[ni][nj] == -1 or arr[ni][nj] == -3): continue
            # if arr[ni][nj] < time + 1: continue
